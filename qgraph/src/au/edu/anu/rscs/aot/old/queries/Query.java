 ...
 * </p>
 * <ul>
 * <li>A subclass of Query, matching the particular condition to fulfill, is
 * constructed with some parameters;</li>
 * <li>Later on, a call to {@code Query.satisfied(Object)} will check the
 * condition and return true if the Query was satistfied;</li>
 ...
 * </ul>
 * <p>
 * Most Query subclasses come with static methods implementing the most common
 * use cases for their class.
 * </p>
 * <p>
 * Queries in the base package apply to non-graph objects.
 * </p>
 * <p>
 * The {@code process()} method parameter type is generic ({@link Object}), but
 * for every particular Query a specific type is required (cf. the javadoc for
 * each specific Query type).
 * </p>
 * <p>
 * Some queries compute a result, accessible through the {@code getResult()}
 ...
 * case is returning a filtered list when the input to {@code process()} is a
 * list of items.
 * </p>
 * 
...
public abstract class Query {
	...
	private static String itemString(Object item) {
		if (item instanceof Element) {
			return ((Element) item).toShortString();
		}
	...
	protected void failed(Object item) {
		...
		String message = "'" + itemString(item) + "' does not satisfy '" + toString() + "'";
		// Log.log("Query: " + message);
		throw new QGraphException(message);
	}

	protected void failed(Object item, Exception e) {
		String message = "'" + itemString(item) + "' does not satisfy '" + toString() + "' [" + e + "]";
		// Log.log("Query: " + message);
		throw new QGraphException(message);
	}

	protected void failed(Object item, String msg) {
		String message = "'" + itemString(item) + "' does not satisfy '" + toString() + "' [" + msg + "]";
		// Log.log("Query: " + message);
		throw new QGraphException(message);
	}
	...
	public String toString() {
		return "[" + this.getClass().getSimpleName() + ", satisfied=" + satisfied + ", result=[" + result + "]]";
	}

	public String userString() {
		return "[" + stateString() + this.getClass().getSimpleName() + "]";
	}
